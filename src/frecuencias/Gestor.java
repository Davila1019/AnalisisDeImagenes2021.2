...
public NumeroComplejo[][] obtenerDatosPorCanal(BufferedImage imagenOriginal, CanalColor color) {
    ...
    public BufferedImage obtenerImagenFrecuenciasRojas(NumeroComplejo[][] datosIO,int w, int h,boolean reAjustarCuadrante) {
    ...
    public BufferedImage obtenerImagenFrecuenciasAzules(NumeroComplejo[][] datosIO,int w, int h,boolean reAjustarCuadrante) {
        /// obtenemos las dimensiones
        int anchoImagen = w;
        int altoImagen = h;
        BufferedImage aux = new BufferedImage(anchoImagen, altoImagen, BufferedImage.TYPE_INT_ARGB);
        ...
        NumeroComplejo[][] transformada = fft.calculateFT(datosIO, false);
            ...
            for (int y = 0; y < aux.getHeight(); y++) {
                for (int x = 0; x < aux.getWidth(); x++) {
                    ...
                    // el que se ecuentre en la imagen 
                    int color1 = aux.getRGB(x, y);
                    int color2 = obtenerColorRealDeFrecuencia(ejeX, ejeY, transformada, HerramientasColor.CanalColor.AZUL);
                    int rgb = HerramientasColor.acumularColor(color1, color2);
                    aux.setRGB(x, y, rgb);
    ...
    public BufferedImage obtenerImagenFrecuenciasVerdes(NumeroComplejo[][] datosIO,int w, int h,boolean reAjustarCuadrante) {
        /// obtenemos las dimensiones
        int anchoImagen = w;
        int altoImagen = h;
        BufferedImage aux = new BufferedImage(anchoImagen, altoImagen, BufferedImage.TYPE_INT_ARGB);
    ...
    private int obtenerColorRealDeFrecuencia(int ejeX, int ejeY, NumeroComplejo[][] transformada, CanalColor canal) {
        ...
        color = Espacial.Espacial.validarLimites(color);
